    ...
    <div class="dt-sidebar__container">
        ...
        <ul class="dt-side-nav">
            ...
            <li class="dt-side-nav__item {{ request()->is($menu->url) ? 'selected' : '' }}">
                <a href="{{  url($menu->url) }}"
                    class="dt-side-nav__link {{ request()->is($menu->url) ? 'active' : '' }}"
                    title="{{ $menu->module_name }}"> <i class="{{ $menu->icon_class }}"></i>
            ...
            <li class="dt-side-nav__item 
                ...
                <a href="javascript:void(0)" class="dt-side-nav__link dt-side-nav__arrow"
                    ...
                    <i class="{{ $menu->icon_class }}"></i> <span
                        class="dt-side-nav__text">{{ $menu->module_name }}</span>
                </a>
                <!-- Sub-menu -->
                <ul class="dt-side-nav__sub-menu">
                    ...
                    <li class="dt-side-nav__item {{ request()->is($submenu->url) ? 'selected' : '' }}">
                        <a href="{{ url($submenu->url) }}"
                            class="dt-side-nav__link {{ request()->is($submenu->url) ? 'active' : '' }}"
                            title="{{ $submenu->module_name }}">
