                ...
                <div class="dt-card__body">
                    ...
                    <form action="{{ route("menu.module.store.or.update") }}" method="post">
                        ...
                        <input type="hidden" name="update_id"
                            value="{{ isset($data['module']) ? $data['module']->id : ''}}">
                        <input type="hidden" name="menu_id" value="{{ $data['menu']->id }}">
                        <div class="form-group required">
                            <label for="type">Type</label>
                            <select name="type" class="form-control selectpicker @error('type') is-invalid @enderror"
                                id="type" onchange="setItemType(this.value)">
                                <option value="">Select Please</option>
                                <option value="1" @isset($data['module'])
                                    {{ $data['module']->type == 1 ? 'selected' : '' }} @endisset
                                    {{ old('type') == 1 ? 'selected' : '' }}>Divider</option>
                                <option value="2" @isset($data['module'])
                                    {{ $data['module']->type == 2 ? 'selected' : '' }} @endisset
                        ...
                        <div class="item_fields d-none">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control @error('url') is-invalid @enderror" name="url"
                                    id="url" placeholder="Enter Module URL"
                            ...
                            <div class="form-group">
                                <label for="icon_class">Font Icon class for the Module <a
                                        href="https://fontawesome.com" target="_blank">(Use a Fontawesome font class)</a></label>
                            ...
                            <div class="form-group required">
                                ...
                                <select name="target"
                                    class="form-control selectpicker  @error('target') is-invalid @enderror"
                                    id="target">
                                    <option value="">Select Please</option>
                                    <option value="_self" @isset($data['module'])
                                        {{ $data['module']->target == '_self' ? 'selected' : '' }} @endisset
                                        {{ old('target') == '_self' ? 'selected' : '' }}>Same Tab</option>
                                    <option value="_blank" @isset($data['module'])
