                        ...
                        <div class="dt-card">
                            ...
                            <div class="dt-card__body">
                                <form id="saveDataForm" method="post">
                                    @csrf
                                    <input type="hidden" name="update_id" value="{{ $permission_data['role']->id }}"
                                        id="update_id">
                                    <div class="row">
                                        <x-form.textbox labelName="Role Name" name="role_name"
                                            value="{{ $permission_data['role']->role_name }}" required="required"
            ...
            if ($('.module:checked').length >= 1) {
                $.ajax({
                    ...
                    success: function (data) {
                        $('#saveDataForm').find('.is-invalid').removeClass(
                        'is-invalid');
                        $('#saveDataForm').find('.error').remove();
                        if (data.status == false) {
                            $.each(data.errors, function (key, value) {
                                $('#saveDataForm input#' + key).addClass(
                                    'is-invalid');
